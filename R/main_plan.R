...
# This plan roughly corresponds to Y&S README Step 3 (Find open reading frames and 
# translate using TransDecoder with blastp for choosing orfs) and Step 4 
# (Clustering). The starting point for our analysis is assembled transcriptomes
# and reference proteome(s), so Steps 1 and 2 are omitted.

# It tests different combinations of hit_frac cutoff and I-values used
# during clustering, and outputs a report comparing summary statistics
# for alignments generated under these values.

# The user should then select the optimal combination of hit_frac cutoff 
# and I-value to use for plan_02 (building and pruning homolog trees).

# Setup drake cache -------------------------------------------------------

if (file.exists(".plan_01_cache")) {
  plan_01_cache <- this_cache(".plan_01_cache")
} else {
  plan_01_cache <- new_cache(".plan_01_cache")
}

# Define basic input values -----------------------------------------------

# Vector of folder names to create in working directory
main_folders <- c("translation", "clustering", "clusters")
...
# Setup folders
plan_01_folders <- drake_plan(
  top_folder = make_dir(dir_name = "folders__", 
                        outfile = file_out(here::here("folders__/.name")))) %>%
  evaluate_plan(rules = list(folders__ = main_folders)) %>%
  bind_plans(drake_plan(
    sub_folder = make_dir(dir_name = "clusters/hit-frac..my_hit_frac.._I..my_i_value.._e5",
                          infile = file_in(here::here("clusters/.name")),
                          outfile = file_out("clusters/hit-frac..my_hit_frac.._I..my_i_value.._e5/.name")))) %>%
  evaluate_plan(
    rules = list (
...
build_blastp_db <- drake_plan(
  blastp_database = baitfindR::build_blast_db(
    ...
    out_name = here::here("translation/arabidopsis_lygodium"), 
    db_type = "prot", 
    other_args = "-parse_seqids",
    outfile = file_out(here::here("translation/arabidopsis_lygodium.phr"))
  )
)
...
run_transdecoder <- drake_plan(
  ...
  longest_orfs = baitfindR::transdecoder_long_orfs(
    ...
    wd = here::here("translation"),
    other_args = "-S",
    # set infile to blast db .phr file to maintain dependency on blast db
    depends = file_in(here::here("translation/arabidopsis_lygodium.phr")),
    outfile = file_out(here::here("translation/transcriptome__.transdecoder_dir/longest_orfs.pep"))),
  
  # Blast candidate long ORFs against the protein database
  orf_blast = baitfindR::blast_p(
    query = file_in(here::here("translation/transcriptome__.transdecoder_dir/longest_orfs.pep")), 
    database = here::here("translation/arabidopsis_lygodium"),
    other_args = c("-max_target_seqs 1", "-evalue 10",  "-num_threads 2"),
    out_file = file_out(here::here("translation/transcriptome__.blastp.outfmt6"))),
  
  # Output the final ORFs, preferentially retaining those with blast hits
  orf_predict = baitfindR::transdecoder_predict(
    transcriptome_file = file_in(here::here("data/transcriptome__")),
    blast_result = file_in(here::here("translation/transcriptome__.blastp.outfmt6")),
    wd = here::here("translation"),
    infile = file_in(here::here("translation/transcriptome__.transdecoder_dir/longest_orfs.pep")),
    outfile = file_out(here::here("translation/transcriptome__.transdecoder.cds"))),
  ...
  seq_with_short_names = baitfindR::fix_names_from_transdecoder(
    transdecoder_output = file_in(here::here("translation/transcriptome__.transdecoder.cds"))),
  
  # Write out fasta files with shortened names
  seq_with_short_names_out = ape::write.FASTA(
    seq_with_short_names_transcriptome__, 
    file = file_out(here::here("translation/transcriptome__.cds.fa"))),
  
  # Reduce redundancy for cds
  # Note that output is put into clustering/
  reduced = baitfindR::cd_hit_est(
    input = file_in(here::here("translation/transcriptome__.cds.fa")), 
    output = file_out(here::here("clustering/transcriptome__.cds.fa.cdhitest")), 
...
concatenate_cdhitest <- drake_plan(
  read = readr::read_file(file_in(here::here("clustering/transcriptome__.cds.fa.cdhitest")))) %>%
  evaluate_plan(rules = list(transcriptome__ = codes)) %>%
  bind_plans(gather_plan(., target = "cdhitest_files")) %>%
  bind_plans(drake_plan(
    concatenate = baitfindR::cat_files(cdhitest_files, 
                                       output_file=file_out(here::here("clustering/all.fa")))))
...
run_allbyall_blast <- drake_plan(
  ...
  all_blast_db = baitfindR::build_blast_db(
    in_seqs = file_in(here::here("clustering/all.fa") ),
    out_name = here::here("clustering/all.fa"),
    other_args = NULL,
    db_type = "nucl",
    outfile = file_out(here::here("clustering/all.fa.nhr"),
                       here::here("clustering/all.fa.nin"),
                       here::here("clustering/all.fa.nsq"))),
  
  # Blast transcriptomes
  blast_transcriptome = baitfindR::blast_n(
    query = file_in(here::here("clustering/transcriptome__.cds.fa.cdhitest")),
    database = here::here("clustering/all.fa"),
    out_file = file_out(here::here("clustering/transcriptome__.allbyall.blast.outfmt6")),
    other_args = c("-evalue 10", "-num_threads 1", "-max_target_seqs 1000"),
    infile = file_in(here::here("clustering/all.fa.nhr")))) %>% 
  evaluate_plan(rules = list(transcriptome__ = codes))

# All-by-all blast: concatenate output of single blast results
concatenate_allbyall_blast <- drake_plan(
  read_blast = readr::read_file(file_in(here::here("clustering/transcriptome__.allbyall.blast.outfmt6")))) %>%
  evaluate_plan(rules = list(transcriptome__ = codes)) %>%
  bind_plans(gather_plan(., target = "cdhitestblast_files")) %>%
  bind_plans(drake_plan(
    combined_cdhitest = baitfindR::cat_files(
      cdhitestblast_files, 
      output_file=file_out(here::here("clustering/all.rawblast")))))
...
run_mcl <- drake_plan (
  ...
  blast_formatted_for_mcl = baitfindR::blast_to_mcl(
    blast_results = file_in(here::here("clustering/all.rawblast")),
    hit_fraction_cutoff = ..MY_HIT_FRAC..,
    outfile = file_out(here::here("clustering/all.rawblast.hit-frac..MY_HIT_FRAC...minusLogEvalue"))),
  
  # run mcl
  mcl_clusters = baitfindR::mcl(
    mcl_input = file_in(here::here("clustering/all.rawblast.hit-frac..MY_HIT_FRAC...minusLogEvalue")),
    i_value = ..MY_I_VALUE..,
    e_value = 5,
    other_args = c("--abc", "-te", "2"),
    mcl_output = file_out(here::here("clustering/hit-frac..MY_HIT_FRAC.._I..MY_I_VALUE.._e5"))),
  ...
  fasta_clusters = baitfindR::write_fasta_files_from_mcl(
    ...
    mcl_outfile = file_in(here::here("clustering/hit-frac..MY_HIT_FRAC.._I..MY_I_VALUE.._e5")),
    minimal_taxa = 4,
    outdir = here::here("clusters/hit-frac..MY_HIT_FRAC.._I..MY_I_VALUE.._e5"),
    get_hash = TRUE,
    overwrite = TRUE)) %>% 
  evaluate_plan(list(
    "..MY_HIT_FRAC.." = hit_frac_list,
...
run_fasta_to_tree <- drake_plan(
  basic_trees = baitfindR::fasta_to_tree(
    overwrite = TRUE,
    seq_folder = here::here("clusters/hit-frac..MY_HIT_FRAC.._I..MY_I_VALUE.._e5"),
    number_cores = 1,
    seq_type = "dna",
    bootstrap = FALSE,
    get_hash = TRUE,
