...
Make small versions of transcriptome files for testing.

The `baitfindR` package comes with the same set of transcriptomes downsized to 5%, but this is too small to generate gene clusters large enough to align trees (i.e., the resulting clusters each have less than four taxa).

This script can be used to make transcriptomes randomly downsized to the fraction set by `keep_frac`.
...
saveRDS(example_transcriptomes, here::here("data_raw/example_transcriptomes.RDS"))
```